...
//! `verdictan spend provider-budget delete` — delete a provider budget.
...
#[derive(Debug, Args)]
pub(crate) struct ProviderBudgetDeleteArgs {
    /// Provider budget id.
    #[arg(long)]
...
pub(crate) async fn run_async(args: ProviderBudgetDeleteArgs) -> Result<(), CliError> {
    if !args.yes {
        return Err(CliError::user(
            "pass --yes to confirm provider budget deletion",
        ));
    }
    ...
    println!("deleted provider budget {}", args.budget_id);
    Ok(())
}
...
mod tests {
    ...
    fn yes_flag_required_error_message() {
        let msg = "pass --yes to confirm provider budget deletion";
    ...
    fn delete_path_formatting() {
        ...
        assert_eq!(path, "/v1/provider-budgets/pb-99");
    }
}
