...
//! `verdictan user assign-role` — assign an IAM role to an org member.
//!
//! # Module wiring
//! Add `pub(crate) mod user_assign_role;` to `cli/src/commands/mod.rs` to activate.
...
#[derive(Debug, Args)]
pub(crate) struct UserAssignRoleArgs {
    /// User id.
    #[arg(long)]
...
pub(crate) async fn run_async(args: UserAssignRoleArgs) -> Result<(), CliError> {
    ...
    let path = format!("/v1/users/{}/roles", args.user_id);
    ...
    println!("assigned role {} to user {}", args.role_id, args.user_id);
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn assign_role_path_formatting() {
        let user_id = "u-42";
        let path = format!("/v1/users/{}/roles", user_id);
        assert_eq!(path, "/v1/users/u-42/roles");
    }

    #[test]
    fn assign_role_body_construction() {
        let role_id = "r-1";
        let assignment_level = "org";
        let target_id = "org-99";
        let body = json!({
            "role_id": role_id,
            "assignment_level": assignment_level,
            "assignment_target_id": target_id,
        });
        assert_eq!(body["role_id"], "r-1");
        assert_eq!(body["assignment_level"], "org");
        assert_eq!(body["assignment_target_id"], "org-99");
    }

    #[test]
    fn team_level_requires_target_id() {
        let level = "team";
        let target: Option<&str> = None;
        let result = match level {
            "team" => target.map(ToOwned::to_owned).ok_or("required"),
    ...
    #[test]
    fn team_level_with_target_id() {
        let level = "team";
        let target: Option<&str> = Some("t-1");
        let result = match level {
            "team" => target.map(ToOwned::to_owned).ok_or("required"),
            _ => Ok("default".to_string()),
        };
    ...
    #[test]
    fn org_level_with_explicit_target() {
        let level = "org";
        let target: Option<&str> = Some("org-explicit");
        let result = match level {
            "org" => Ok(target.unwrap_or("fallback").to_string()),
            _ => Err("invalid"),
        };
    ...
    #[test]
    fn invalid_assignment_level() {
        let level = "invalid";
        let is_valid = matches!(level, "org" | "team");
        assert!(!is_valid);
    }
}
