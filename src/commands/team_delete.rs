...
//! `verdictan team delete` — delete a team.
//!
//! # Module wiring
//! Add `pub(crate) mod team_delete;` to `cli/src/commands/mod.rs` to activate.
...
pub(crate) async fn run_async(args: TeamDeleteArgs) -> Result<(), CliError> {
    if !args.yes {
        return Err(CliError::user(format!(
            "pass --yes to confirm deleting team {}",
            args.team_id
        )));
    }
    ...
    println!("deleted team {}", args.team_id);
    Ok(())
}
...
mod tests {
    ...
    fn yes_flag_required_error_message() {
        let team_id = "t-42";
        let msg = format!("pass --yes to confirm deleting team {}", team_id);
