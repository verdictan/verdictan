...
//! `verdictan role delete` — delete an IAM role.
//!
//! # Module wiring
//! Add `pub(crate) mod role_delete;` to `cli/src/commands/mod.rs` to activate.
...
#[derive(Debug, Args)]
pub(crate) struct RoleDeleteArgs {
    /// Role id.
    #[arg(long)]
...
pub(crate) async fn run_async(args: RoleDeleteArgs) -> Result<(), CliError> {
    if !args.yes {
        return Err(CliError::user(format!(
            "pass --yes to confirm deleting role {}",
            args.role_id
        )));
    }
    ...
    println!("deleted role {}", args.role_id);
    Ok(())
}
...
mod tests {
    ...
    fn yes_flag_required_error_message() {
        let role_id = "r-42";
        let msg = format!("pass --yes to confirm deleting role {}", role_id);
