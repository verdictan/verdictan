...
pub static HTTP_RUNTIME: HttpRuntime = HttpRuntime;

pub struct HttpRuntime;

impl super::super::VerdictanRuntime for HttpRuntime {
    ...
    fn validate_config(&self, config: &Value) -> Result<(), CliError> {
        let endpoint = config
            ...
            .ok_or_else(|| CliError::user("base_url is required for http runtime"))?;
...
impl VerdictanNetworkAdapterRuntime for HttpRuntime {
    ...
    fn validate_endpoint(&self, endpoint: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "http runtime requires a base_url starting with http:// or https://",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_http() {
        assert_eq!(HTTP_RUNTIME.runtime_id(), "http");
    }

    #[test]
    fn adapter_id_returns_http() {
        assert_eq!(HTTP_RUNTIME.adapter_id(), "http");
    }

    #[test]
    fn validate_config_accepts_http() {
        let config = json!({"base_url": "http://localhost:8080"});
        assert!(HTTP_RUNTIME.validate_config(&config).is_ok());
    }
    ...
    fn validate_config_rejects_missing_base_url() {
        let config = json!({});
        assert!(HTTP_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_invalid_scheme() {
        let config = json!({"base_url": "ws://host"});
        assert!(HTTP_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_accepts_http_and_https() {
        assert!(HTTP_RUNTIME.validate_endpoint("http://host").is_ok());
        assert!(HTTP_RUNTIME.validate_endpoint("https://host").is_ok());
    }

    #[test]
    fn validate_endpoint_rejects_other_schemes() {
        assert!(HTTP_RUNTIME.validate_endpoint("ftp://host").is_err());
    }

    #[test]
    fn requires_model_is_false() {
        assert!(!HTTP_RUNTIME.requires_model());
    }

    #[test]
    fn auth_optional_is_true() {
        assert!(HTTP_RUNTIME.auth_optional());
    }

    #[test]
    fn build_request_passes_through() {
        let input = json!({"body": "data"});
        assert_eq!(
            HTTP_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        let req = json!({"test": 1});
        assert_eq!(HTTP_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"status": 200});
        assert_eq!(HTTP_RUNTIME.translate_response(&resp).unwrap(), resp);
    }

    #[test]
    fn does_not_support_streaming() {
        assert!(!HTTP_RUNTIME.supports_streaming());
    }

    #[test]
    fn does_not_support_tools() {
        assert!(!HTTP_RUNTIME.supports_tools());
    }
