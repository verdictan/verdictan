...
pub static LOCALAI_RUNTIME: LocalAiRuntime = LocalAiRuntime;
pub struct LocalAiRuntime;
impl super::super::VerdictanRuntime for LocalAiRuntime {
    ...
    fn validate_config(&self, config: &Value) -> Result<(), CliError> {
        let base_url = config
            ...
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| CliError::user("base_url is required for localai runtime"))?;
...
impl VerdictanUpstreamRuntime for LocalAiRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        if base_url.starts_with("http://") || base_url.starts_with("https://") {
            ...
            Err(CliError::user(
                "localai runtime requires an http:// or https:// base_url",
            ))
        }
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_localai() {
        assert_eq!(LOCALAI_RUNTIME.runtime_id(), "localai");
    }

    #[test]
    fn provider_kind_returns_localai() {
        assert_eq!(LOCALAI_RUNTIME.provider_kind(), "localai");
    }

    #[test]
    fn validate_config_accepts_http_base_url() {
        let config = json!({"base_url": "http://localhost:8080"});
        assert!(LOCALAI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_accepts_https_base_url() {
        let config = json!({"base_url": "https://localai.example.com"});
        assert!(LOCALAI_RUNTIME.validate_config(&config).is_ok());
    }
    ...
    fn validate_config_rejects_missing_base_url() {
        let config = json!({});
        let err = LOCALAI_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_base_url() {
        ...
        assert!(LOCALAI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(LOCALAI_RUNTIME
            .validate_endpoint_url("https://localai.example.com")
            .is_ok());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&LOCALAI_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn auth_optional_is_true() {
        assert!(LOCALAI_RUNTIME.auth_optional());
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = LOCALAI_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = LOCALAI_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = LOCALAI_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(LOCALAI_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(LOCALAI_RUNTIME.supports_tools());
    }
}
