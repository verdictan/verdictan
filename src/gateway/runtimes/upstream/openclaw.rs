...
pub static OPENCLAW_RUNTIME: OpenclawRuntime = OpenclawRuntime;

pub struct OpenclawRuntime;

impl super::super::VerdictanRuntime for OpenclawRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for openclaw runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for OpenclawRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "openclaw runtime requires an http:// or https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_openclaw() {
        assert_eq!(OPENCLAW_RUNTIME.runtime_id(), "openclaw");
    }

    #[test]
    fn provider_kind_returns_openclaw() {
        assert_eq!(OPENCLAW_RUNTIME.provider_kind(), "openclaw");
    }

    #[test]
    fn validate_config_accepts_valid() {
        let config = json!({"model": "m1", "base_url": "https://api.openclaw.io"});
        assert!(OPENCLAW_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_accepts_http() {
        let config = json!({"model": "m1", "base_url": "http://localhost:8080"});
        assert!(OPENCLAW_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.openclaw.io"});
        assert!(OPENCLAW_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "m1"});
        assert!(OPENCLAW_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_invalid_scheme() {
        let config = json!({"model": "m1", "base_url": "ftp://host"});
        assert!(OPENCLAW_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_url_accepts_http_and_https() {
        assert!(OPENCLAW_RUNTIME
            .validate_endpoint_url("http://localhost")
            .is_ok());
        assert!(OPENCLAW_RUNTIME
            .validate_endpoint_url("https://api.io")
            .is_ok());
    }

    #[test]
    fn validate_endpoint_url_rejects_bare() {
        assert!(OPENCLAW_RUNTIME.validate_endpoint_url("api.io").is_err());
    }

    #[test]
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&OPENCLAW_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        assert_eq!(
            OPENCLAW_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(OPENCLAW_RUNTIME.supports_streaming());
        assert!(OPENCLAW_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.openclaw.io"});
        assert!(OPENCLAW_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(OPENCLAW_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(OPENCLAW_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        assert_eq!(
            OPENCLAW_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
}
