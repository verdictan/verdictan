...
pub static VERTEX_RUNTIME: VertexRuntime = VertexRuntime;

pub struct VertexRuntime;

impl super::super::VerdictanRuntime for VertexRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for vertex runtime"
                    )))
                }
            }
        }
    ...
    fn default_path_template(&self) -> Option<&'static str> {
        Some("/v1/projects/{project}/locations/{location}/publishers/google/models/{model}:generateContent")
    }
}

impl VerdictanUpstreamRuntime for VertexRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "vertex runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(VERTEX_RUNTIME.runtime_id(), "vertex");
    }

    #[test]
    fn provider_kind() {
        assert_eq!(VERTEX_RUNTIME.provider_kind(), "vertex");
    }

    #[test]
    fn validate_config_valid() {
        let config = json!({"model": "gemini-pro", "base_url": "https://us-central1-aiplatform.googleapis.com"});
        assert!(VERTEX_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_missing_fields() {
        assert!(VERTEX_RUNTIME
    ...
    fn validate_endpoint_url_https() {
        assert!(VERTEX_RUNTIME
            .validate_endpoint_url("https://us-central1-aiplatform.googleapis.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_http_rejected() {
        assert!(VERTEX_RUNTIME
            .validate_endpoint_url("http://us-central1-aiplatform.googleapis.com")
            .is_err());
    }

    #[test]
    fn default_path_template_value() {
        let path = VerdictanRuntime::default_path_template(&VERTEX_RUNTIME).unwrap();
    ...
    fn supports_streaming_and_tools() {
        assert!(VERTEX_RUNTIME.supports_streaming());
        assert!(VERTEX_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "gemini-pro"});
        assert!(VERTEX_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config =
            json!({"model": "", "base_url": "https://us-central1-aiplatform.googleapis.com"});
        assert!(VERTEX_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(VERTEX_RUNTIME
            .validate_endpoint_url("us-central1-aiplatform.googleapis.com")
            .is_err());
    }

    #[test]
    fn build_request_passes_through() {
        let input = json!({"contents": [{"role": "user", "parts": [{"text": "hi"}]}]});
        assert_eq!(
            VERTEX_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(VERTEX_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"candidates": []});
        assert_eq!(VERTEX_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        assert_eq!(
            VERTEX_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
}
