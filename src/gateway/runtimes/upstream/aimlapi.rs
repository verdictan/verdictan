...
pub static AIMLAPI_RUNTIME: AimlApiRuntime = AimlApiRuntime;

pub struct AimlApiRuntime;

impl super::super::VerdictanRuntime for AimlApiRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for aimlapi runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for AimlApiRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "aimlapi runtime requires an http:// or https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_aimlapi() {
        assert_eq!(AIMLAPI_RUNTIME.runtime_id(), "aimlapi");
    }

    #[test]
    fn provider_kind_returns_aimlapi() {
        assert_eq!(AIMLAPI_RUNTIME.provider_kind(), "aimlapi");
    }

    #[test]
    fn validate_config_accepts_http() {
        let config = json!({"model": "m", "base_url": "http://api.aimlapi.com"});
        assert!(AIMLAPI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_accepts_https() {
        let config = json!({"model": "m", "base_url": "https://api.aimlapi.com"});
        assert!(AIMLAPI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.aimlapi.com"});
        assert!(AIMLAPI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_invalid_scheme() {
        let config = json!({"model": "m", "base_url": "ftp://host"});
        assert!(AIMLAPI_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_url_accepts_http_and_https() {
        assert!(AIMLAPI_RUNTIME.validate_endpoint_url("http://host").is_ok());
        assert!(AIMLAPI_RUNTIME
    ...
    fn validate_endpoint_url_rejects_other_schemes() {
        assert!(AIMLAPI_RUNTIME.validate_endpoint_url("ws://host").is_err());
    }

    #[test]
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&AIMLAPI_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(AIMLAPI_RUNTIME.supports_streaming());
        assert!(AIMLAPI_RUNTIME.supports_tools());
    }
    ...
    fn build_request_passes_through() {
        ...
        assert_eq!(
            AIMLAPI_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(AIMLAPI_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(AIMLAPI_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        assert_eq!(
            AIMLAPI_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.aimlapi.com"});
        assert!(AIMLAPI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(AIMLAPI_RUNTIME
            .validate_endpoint_url("api.aimlapi.com")
            .is_err());
    }
}
