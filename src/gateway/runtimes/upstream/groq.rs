...
pub static GROQ_RUNTIME: GroqRuntime = GroqRuntime;

pub struct GroqRuntime;

impl super::super::VerdictanRuntime for GroqRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for groq runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for GroqRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user("groq runtime requires an https:// base_url"))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_groq() {
        assert_eq!(GROQ_RUNTIME.runtime_id(), "groq");
    }

    #[test]
    fn provider_kind_returns_groq() {
        assert_eq!(GROQ_RUNTIME.provider_kind(), "groq");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(GROQ_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.groq.com"});
        let err = GROQ_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama3-8b-8192"});
        let err = GROQ_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.groq.com"});
        assert!(GROQ_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(GROQ_RUNTIME
            .validate_endpoint_url("https://api.groq.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(GROQ_RUNTIME
            .validate_endpoint_url("http://api.groq.com")
            .is_err());
    }

    #[test]
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(GROQ_RUNTIME.validate_endpoint_url("api.groq.com").is_err());
    }

    #[test]
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&GROQ_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = GROQ_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = GROQ_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = GROQ_RUNTIME.translate_response(&response).unwrap();
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        let result = GROQ_RUNTIME.normalize_upstream_response(&response).unwrap();
        assert_eq!(result, response);
    }

    #[test]
    fn supports_streaming_is_true() {
        assert!(GROQ_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(GROQ_RUNTIME.supports_tools());
    }
}
