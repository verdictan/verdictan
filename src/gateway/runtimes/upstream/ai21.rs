...
pub static AI21_RUNTIME: Ai21Runtime = Ai21Runtime;

pub struct Ai21Runtime;

impl super::super::VerdictanRuntime for Ai21Runtime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for ai21 runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for Ai21Runtime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user("ai21 runtime requires an https:// base_url"))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(AI21_RUNTIME.runtime_id(), "ai21");
    }

    #[test]
    fn provider_kind() {
        assert_eq!(AI21_RUNTIME.provider_kind(), "ai21");
    }

    #[test]
    fn validate_config_valid() {
        let config = json!({"model": "jamba-instruct", "base_url": "https://api.ai21.com"});
        assert!(AI21_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_missing_model() {
        let config = json!({"base_url": "https://api.ai21.com"});
        let err = AI21_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_missing_base_url() {
        let config = json!({"model": "jamba-instruct"});
        let err = AI21_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_http_rejected() {
        let config = json!({"model": "m", "base_url": "http://insecure.example.com"});
        assert!(AI21_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_url_https() {
        assert!(AI21_RUNTIME
            .validate_endpoint_url("https://api.ai21.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_http_rejected() {
        assert!(AI21_RUNTIME
            .validate_endpoint_url("http://api.ai21.com")
            .is_err());
    }
    ...
    fn default_path_template_value() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&AI21_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passthrough() {
        ...
        let result = AI21_RUNTIME.build_request(&json!({}), &input).unwrap();
        assert_eq!(result, input);
    }

    #[test]
    fn supports_streaming_and_tools() {
        assert!(AI21_RUNTIME.supports_streaming());
        assert!(AI21_RUNTIME.supports_tools());
    }

    #[test]
    fn normalize_upstream_response_passthrough() {
        let resp = json!({"id": "123"});
        assert_eq!(
            AI21_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(AI21_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(AI21_RUNTIME.translate_response(&resp).unwrap(), resp);
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.ai21.com"});
        assert!(AI21_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(AI21_RUNTIME.validate_endpoint_url("api.ai21.com").is_err());
    }
}
