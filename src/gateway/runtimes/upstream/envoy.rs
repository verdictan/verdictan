...
pub static ENVOY_RUNTIME: EnvoyRuntime = EnvoyRuntime;

pub struct EnvoyRuntime;

impl super::super::VerdictanRuntime for EnvoyRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for envoy runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for EnvoyRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "envoy runtime requires an http:// or https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_envoy() {
        assert_eq!(ENVOY_RUNTIME.runtime_id(), "envoy");
    }

    #[test]
    fn provider_kind_returns_envoy() {
        assert_eq!(ENVOY_RUNTIME.provider_kind(), "envoy");
    }
    ...
    fn validate_config_accepts_valid_https_config() {
        let config = json!({
            "model": "gpt-4",
            "base_url": "https://envoy-proxy.example.com"
        });
        assert!(ENVOY_RUNTIME.validate_config(&config).is_ok());
    }
    ...
    fn validate_config_accepts_http_base_url() {
        let config = json!({
            "model": "gpt-4",
            "base_url": "http://envoy-proxy.internal:8080"
        });
        assert!(ENVOY_RUNTIME.validate_config(&config).is_ok());
    }
    ...
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://envoy.example.com"});
        let err = ENVOY_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://envoy.example.com"});
        assert!(ENVOY_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(ENVOY_RUNTIME
            .validate_endpoint_url("https://envoy.example.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_accepts_http() {
        assert!(ENVOY_RUNTIME
            .validate_endpoint_url("http://envoy.internal:8080")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(ENVOY_RUNTIME
            .validate_endpoint_url("envoy.example.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_ftp() {
        assert!(ENVOY_RUNTIME
            .validate_endpoint_url("ftp://envoy.example.com")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&ENVOY_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = ENVOY_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = ENVOY_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = ENVOY_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(ENVOY_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(ENVOY_RUNTIME.supports_tools());
    }
}
