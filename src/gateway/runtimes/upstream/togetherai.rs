...
pub static TOGETHERAI_RUNTIME: TogetherAiRuntime = TogetherAiRuntime;

pub struct TogetherAiRuntime;

impl super::super::VerdictanRuntime for TogetherAiRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for togetherai runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for TogetherAiRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "togetherai runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_togetherai() {
        assert_eq!(TOGETHERAI_RUNTIME.runtime_id(), "togetherai");
    }

    #[test]
    fn provider_kind_returns_togetherai() {
        assert_eq!(TOGETHERAI_RUNTIME.provider_kind(), "togetherai");
    }

    #[test]
    fn validate_config_accepts_valid() {
        let config = json!({"model": "m", "base_url": "https://api.together.xyz"});
        assert!(TOGETHERAI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_http() {
        let config = json!({"model": "m", "base_url": "http://api.together.xyz"});
        assert!(TOGETHERAI_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.together.xyz"});
        assert!(TOGETHERAI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_only_accepts_https() {
        assert!(TOGETHERAI_RUNTIME
            .validate_endpoint_url("https://api.together.xyz")
            .is_ok());
        assert!(TOGETHERAI_RUNTIME
            .validate_endpoint_url("http://api.together.xyz")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&TOGETHERAI_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(TOGETHERAI_RUNTIME.supports_streaming());
        assert!(TOGETHERAI_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.together.xyz"});
        assert!(TOGETHERAI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(TOGETHERAI_RUNTIME
            .validate_endpoint_url("api.together.xyz")
            .is_err());
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(TOGETHERAI_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(TOGETHERAI_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
