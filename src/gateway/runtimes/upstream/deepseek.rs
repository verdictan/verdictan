...
pub static DEEPSEEK_RUNTIME: DeepSeekRuntime = DeepSeekRuntime;

pub struct DeepSeekRuntime;

impl super::super::VerdictanRuntime for DeepSeekRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for deepseek runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for DeepSeekRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "deepseek runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(DEEPSEEK_RUNTIME.runtime_id(), "deepseek");
    }

    #[test]
    fn provider_kind() {
        assert_eq!(DEEPSEEK_RUNTIME.provider_kind(), "deepseek");
    }

    #[test]
    fn validate_config_valid() {
        let config = json!({"model": "deepseek-chat", "base_url": "https://api.deepseek.com"});
        assert!(DEEPSEEK_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_missing_fields() {
        assert!(DEEPSEEK_RUNTIME
    ...
    fn validate_endpoint_url_https() {
        assert!(DEEPSEEK_RUNTIME
            .validate_endpoint_url("https://api.deepseek.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_http_rejected() {
        assert!(DEEPSEEK_RUNTIME
            .validate_endpoint_url("http://api.deepseek.com")
            .is_err());
    }
    ...
    fn default_path_template_value() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&DEEPSEEK_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(DEEPSEEK_RUNTIME.supports_streaming());
        assert!(DEEPSEEK_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.deepseek.com"});
        assert!(DEEPSEEK_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(DEEPSEEK_RUNTIME
            .validate_endpoint_url("api.deepseek.com")
            .is_err());
    }
    ...
    fn build_request_passes_through() {
        ...
        assert_eq!(
            DEEPSEEK_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(DEEPSEEK_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(DEEPSEEK_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        assert_eq!(
            DEEPSEEK_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
}
