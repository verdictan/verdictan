...
pub static CLOUDFLARE_GATEWAY_RUNTIME: CloudflareGatewayRuntime = CloudflareGatewayRuntime;

pub struct CloudflareGatewayRuntime;

impl super::super::VerdictanRuntime for CloudflareGatewayRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for cloudflare-gateway runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for CloudflareGatewayRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "cloudflare-gateway runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    fn runtime_id() {
        assert_eq!(
            CLOUDFLARE_GATEWAY_RUNTIME.runtime_id(),
            "cloudflare-gateway"
        );
    }
    ...
    fn provider_kind() {
        assert_eq!(
            CLOUDFLARE_GATEWAY_RUNTIME.provider_kind(),
            "cloudflare-gateway"
        );
    }
    ...
    fn validate_config_valid() {
        let config = json!({"model": "gpt-4o", "base_url": "https://gateway.ai.cloudflare.com"});
        assert!(CLOUDFLARE_GATEWAY_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_missing_fields() {
        assert!(CLOUDFLARE_GATEWAY_RUNTIME
    ...
    fn validate_endpoint_url_https() {
        assert!(CLOUDFLARE_GATEWAY_RUNTIME
            .validate_endpoint_url("https://gateway.ai.cloudflare.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_http_rejected() {
        assert!(CLOUDFLARE_GATEWAY_RUNTIME
            .validate_endpoint_url("http://gateway.ai.cloudflare.com")
            .is_err());
    }
    ...
    fn default_path_template_value() {
        assert_eq!(
            <CloudflareGatewayRuntime as VerdictanRuntime>::default_path_template(
                &CLOUDFLARE_GATEWAY_RUNTIME
            ),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(CLOUDFLARE_GATEWAY_RUNTIME.supports_streaming());
        assert!(CLOUDFLARE_GATEWAY_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "gpt-4o"});
        assert!(CLOUDFLARE_GATEWAY_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://gateway.ai.cloudflare.com"});
        assert!(CLOUDFLARE_GATEWAY_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(CLOUDFLARE_GATEWAY_RUNTIME
            .validate_endpoint_url("gateway.ai.cloudflare.com")
            .is_err());
    }
    ...
    fn upstream_default_path_template_value() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&CLOUDFLARE_GATEWAY_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
}
