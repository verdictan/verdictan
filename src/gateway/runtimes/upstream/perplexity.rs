...
pub static PERPLEXITY_RUNTIME: PerplexityRuntime = PerplexityRuntime;

pub struct PerplexityRuntime;

impl super::super::VerdictanRuntime for PerplexityRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for perplexity runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for PerplexityRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "perplexity runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_perplexity() {
        assert_eq!(PERPLEXITY_RUNTIME.runtime_id(), "perplexity");
    }

    #[test]
    fn provider_kind_returns_perplexity() {
        assert_eq!(PERPLEXITY_RUNTIME.provider_kind(), "perplexity");
    }

    #[test]
    fn validate_config_accepts_valid() {
        let config = json!({"model": "sonar", "base_url": "https://api.perplexity.ai"});
        assert!(PERPLEXITY_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_http() {
        let config = json!({"model": "sonar", "base_url": "http://api.perplexity.ai"});
        assert!(PERPLEXITY_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.perplexity.ai"});
        assert!(PERPLEXITY_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "sonar"});
        assert!(PERPLEXITY_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_url_only_accepts_https() {
        assert!(PERPLEXITY_RUNTIME
            .validate_endpoint_url("https://api.perplexity.ai")
            .is_ok());
        assert!(PERPLEXITY_RUNTIME
            .validate_endpoint_url("http://api.perplexity.ai")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&PERPLEXITY_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(PERPLEXITY_RUNTIME.supports_streaming());
        assert!(PERPLEXITY_RUNTIME.supports_tools());
    }
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.perplexity.ai"});
        assert!(PERPLEXITY_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(PERPLEXITY_RUNTIME
            .validate_endpoint_url("api.perplexity.ai")
            .is_err());
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(PERPLEXITY_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(PERPLEXITY_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
