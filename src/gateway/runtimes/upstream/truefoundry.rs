...
pub static TRUEFOUNDRY_RUNTIME: TrueFoundryRuntime = TrueFoundryRuntime;
pub struct TrueFoundryRuntime;
impl super::super::VerdictanRuntime for TrueFoundryRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for truefoundry runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for TrueFoundryRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        if base_url.starts_with("https://") {
            ...
            Err(CliError::user(
                "truefoundry runtime requires an https:// base_url",
            ))
        }
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(TRUEFOUNDRY_RUNTIME.runtime_id(), "truefoundry");
    }

    #[test]
    fn provider_kind() {
        assert_eq!(TRUEFOUNDRY_RUNTIME.provider_kind(), "truefoundry");
    }

    #[test]
    fn validate_config_valid() {
        let config = json!({"model": "llama3", "base_url": "https://llm.truefoundry.com"});
        assert!(TRUEFOUNDRY_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_missing_fields() {
        assert!(TRUEFOUNDRY_RUNTIME
    ...
    fn validate_endpoint_url_https() {
        assert!(TRUEFOUNDRY_RUNTIME
            .validate_endpoint_url("https://llm.truefoundry.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_http_rejected() {
        assert!(TRUEFOUNDRY_RUNTIME
            .validate_endpoint_url("http://llm.truefoundry.com")
            .is_err());
    }
    ...
    fn default_path_template_value() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&TRUEFOUNDRY_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn supports_streaming_and_tools() {
        assert!(TRUEFOUNDRY_RUNTIME.supports_streaming());
        assert!(TRUEFOUNDRY_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama3"});
        assert!(TRUEFOUNDRY_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://llm.truefoundry.com"});
        assert!(TRUEFOUNDRY_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(TRUEFOUNDRY_RUNTIME
            .validate_endpoint_url("llm.truefoundry.com")
            .is_err());
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(TRUEFOUNDRY_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(TRUEFOUNDRY_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
