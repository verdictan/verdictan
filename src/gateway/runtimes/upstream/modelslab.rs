...
pub static MODELSLAB_RUNTIME: ModelslabRuntime = ModelslabRuntime;

pub struct ModelslabRuntime;

impl super::super::VerdictanRuntime for ModelslabRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for modelslab runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for ModelslabRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "modelslab runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_modelslab() {
        assert_eq!(MODELSLAB_RUNTIME.runtime_id(), "modelslab");
    }

    #[test]
    fn provider_kind_returns_modelslab() {
        assert_eq!(MODELSLAB_RUNTIME.provider_kind(), "modelslab");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(MODELSLAB_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://modelslab.com"});
        let err = MODELSLAB_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "sdxl"});
        let err = MODELSLAB_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "  ", "base_url": "https://modelslab.com"});
        assert!(MODELSLAB_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(MODELSLAB_RUNTIME
            .validate_endpoint_url("https://modelslab.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(MODELSLAB_RUNTIME
            .validate_endpoint_url("http://modelslab.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(MODELSLAB_RUNTIME
            .validate_endpoint_url("modelslab.com")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&MODELSLAB_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = MODELSLAB_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = MODELSLAB_RUNTIME.execute(&config, &request).unwrap();
        assert_eq!(result, request);
    }

    #[test]
    fn translate_response_passes_through() {
        let response = json!({"output": ["https://example.com/img.png"]});
        let result = MODELSLAB_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_false() {
        assert!(!MODELSLAB_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_false() {
        assert!(!MODELSLAB_RUNTIME.supports_tools());
    }
}
