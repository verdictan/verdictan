...
pub static CLOUDFLARE_AI_RUNTIME: CloudflareAiRuntime = CloudflareAiRuntime;

pub struct CloudflareAiRuntime;

impl super::super::VerdictanRuntime for CloudflareAiRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for cloudflare-ai runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for CloudflareAiRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "cloudflare-ai runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_cloudflare_ai() {
        assert_eq!(CLOUDFLARE_AI_RUNTIME.runtime_id(), "cloudflare-ai");
    }

    #[test]
    fn provider_kind_returns_cloudflare_ai() {
        assert_eq!(CLOUDFLARE_AI_RUNTIME.provider_kind(), "cloudflare-ai");
    }
    ...
    fn validate_config_accepts_valid_config() {
        let config = json!({
            ...
            "base_url": "https://api.cloudflare.com/client/v4/accounts/abc123/ai"
        });
        assert!(CLOUDFLARE_AI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.cloudflare.com"});
        let err = CLOUDFLARE_AI_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "@cf/meta/llama-2-7b-chat-int8"});
        let err = CLOUDFLARE_AI_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "  ", "base_url": "https://api.cloudflare.com"});
        assert!(CLOUDFLARE_AI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(CLOUDFLARE_AI_RUNTIME
            .validate_endpoint_url("https://api.cloudflare.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(CLOUDFLARE_AI_RUNTIME
            .validate_endpoint_url("http://api.cloudflare.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(CLOUDFLARE_AI_RUNTIME
            .validate_endpoint_url("api.cloudflare.com")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&CLOUDFLARE_AI_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = CLOUDFLARE_AI_RUNTIME
    ...
    fn execute_passes_through() {
        ...
        let result = CLOUDFLARE_AI_RUNTIME.execute(&config, &request).unwrap();
        assert_eq!(result, request);
    }

    #[test]
    fn translate_response_passes_through() {
        let response = json!({"result": {"response": "hello"}});
        let result = CLOUDFLARE_AI_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(CLOUDFLARE_AI_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(CLOUDFLARE_AI_RUNTIME.supports_tools());
    }
}
