...
pub static MISTRAL_RUNTIME: MistralRuntime = MistralRuntime;

pub struct MistralRuntime;

impl super::super::VerdictanRuntime for MistralRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for mistral runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for MistralRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "mistral runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_mistral() {
        assert_eq!(MISTRAL_RUNTIME.runtime_id(), "mistral");
    }

    #[test]
    fn provider_kind_returns_mistral() {
        assert_eq!(MISTRAL_RUNTIME.provider_kind(), "mistral");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(MISTRAL_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.mistral.ai"});
        let err = MISTRAL_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "mistral-large-latest"});
        let err = MISTRAL_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "  ", "base_url": "https://api.mistral.ai"});
        assert!(MISTRAL_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(MISTRAL_RUNTIME
            .validate_endpoint_url("https://api.mistral.ai")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(MISTRAL_RUNTIME
            .validate_endpoint_url("http://api.mistral.ai")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(MISTRAL_RUNTIME
            .validate_endpoint_url("api.mistral.ai")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&MISTRAL_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = MISTRAL_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = MISTRAL_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = MISTRAL_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(MISTRAL_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(MISTRAL_RUNTIME.supports_tools());
    }
}
