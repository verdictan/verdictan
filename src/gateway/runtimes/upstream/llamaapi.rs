...
pub static LLAMAAPI_RUNTIME: LlamaApiRuntime = LlamaApiRuntime;
pub struct LlamaApiRuntime;
impl super::super::VerdictanRuntime for LlamaApiRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for llamaapi runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for LlamaApiRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        if base_url.starts_with("https://") {
            ...
            Err(CliError::user(
                "llamaapi runtime requires an https:// base_url",
            ))
        }
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_llamaapi() {
        assert_eq!(LLAMAAPI_RUNTIME.runtime_id(), "llamaapi");
    }

    #[test]
    fn provider_kind_returns_llamaapi() {
        assert_eq!(LLAMAAPI_RUNTIME.provider_kind(), "llamaapi");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(LLAMAAPI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.llama-api.com"});
        let err = LLAMAAPI_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama3.1-405b"});
        let err = LLAMAAPI_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.llama-api.com"});
        assert!(LLAMAAPI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_http_base_url() {
        let config = json!({
            "model": "llama3.1-405b",
            "base_url": "http://api.llama-api.com"
        });
        assert!(LLAMAAPI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(LLAMAAPI_RUNTIME
            .validate_endpoint_url("https://api.llama-api.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(LLAMAAPI_RUNTIME
            .validate_endpoint_url("http://api.llama-api.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(LLAMAAPI_RUNTIME
            .validate_endpoint_url("api.llama-api.com")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&LLAMAAPI_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = LLAMAAPI_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = LLAMAAPI_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = LLAMAAPI_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(LLAMAAPI_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(LLAMAAPI_RUNTIME.supports_tools());
    }
}
