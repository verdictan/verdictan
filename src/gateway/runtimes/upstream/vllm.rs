...
pub static VLLM_RUNTIME: VllmRuntime = VllmRuntime;

pub struct VllmRuntime;

impl super::super::VerdictanRuntime for VllmRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for vllm runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for VllmRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "vllm runtime requires an http:// or https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_vllm() {
        assert_eq!(VLLM_RUNTIME.runtime_id(), "vllm");
    }

    #[test]
    fn provider_kind_returns_vllm() {
        assert_eq!(VLLM_RUNTIME.provider_kind(), "vllm");
    }

    #[test]
    fn validate_config_accepts_http() {
        let config = json!({"model": "llama", "base_url": "http://localhost:8000"});
        assert!(VLLM_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_accepts_https() {
        let config = json!({"model": "llama", "base_url": "https://vllm.host"});
        assert!(VLLM_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "http://localhost:8000"});
        assert!(VLLM_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama"});
        assert!(VLLM_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_invalid_scheme() {
        let config = json!({"model": "llama", "base_url": "ftp://host"});
        assert!(VLLM_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_http_and_https() {
        assert!(VLLM_RUNTIME
            ...
            .is_ok());
        assert!(VLLM_RUNTIME
            .validate_endpoint_url("https://vllm.host")
            .is_ok());
    }

    #[test]
    fn validate_endpoint_url_rejects_other_schemes() {
        assert!(VLLM_RUNTIME.validate_endpoint_url("ws://host").is_err());
    }

    #[test]
    fn auth_optional_is_true() {
        assert!(VLLM_RUNTIME.auth_optional());
    }

    #[test]
    fn supports_streaming_and_tools() {
        assert!(VLLM_RUNTIME.supports_streaming());
        assert!(VLLM_RUNTIME.supports_tools());
    }

    #[test]
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&VLLM_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        assert_eq!(
            VLLM_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(VLLM_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(VLLM_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        assert_eq!(
            VLLM_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "http://localhost:8000"});
        assert!(VLLM_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(VLLM_RUNTIME
            .validate_endpoint_url("localhost:8000")
            .is_err());
    }
}
