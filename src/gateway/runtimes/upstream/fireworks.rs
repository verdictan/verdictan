...
pub static FIREWORKS_RUNTIME: FireworksRuntime = FireworksRuntime;

pub struct FireworksRuntime;

impl super::super::VerdictanRuntime for FireworksRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for fireworks runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for FireworksRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "fireworks runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_fireworks() {
        assert_eq!(FIREWORKS_RUNTIME.runtime_id(), "fireworks");
    }

    #[test]
    fn provider_kind_returns_fireworks() {
        assert_eq!(FIREWORKS_RUNTIME.provider_kind(), "fireworks");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(FIREWORKS_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.fireworks.ai"});
        let err = FIREWORKS_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama-v3-70b-instruct"});
        let err = FIREWORKS_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "  ", "base_url": "https://api.fireworks.ai"});
        assert!(FIREWORKS_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_http_base_url() {
        let config = json!({
            "model": "llama-v3",
            "base_url": "http://api.fireworks.ai"
        });
        assert!(FIREWORKS_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(FIREWORKS_RUNTIME
            .validate_endpoint_url("https://api.fireworks.ai")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(FIREWORKS_RUNTIME
            .validate_endpoint_url("http://api.fireworks.ai")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(FIREWORKS_RUNTIME
            .validate_endpoint_url("api.fireworks.ai")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&FIREWORKS_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = FIREWORKS_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = FIREWORKS_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = FIREWORKS_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(FIREWORKS_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(FIREWORKS_RUNTIME.supports_tools());
    }
}
