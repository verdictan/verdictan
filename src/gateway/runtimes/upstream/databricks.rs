...
pub static DATABRICKS_RUNTIME: DatabricksRuntime = DatabricksRuntime;

pub struct DatabricksRuntime;

impl super::super::VerdictanRuntime for DatabricksRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for databricks runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for DatabricksRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "databricks runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_databricks() {
        assert_eq!(DATABRICKS_RUNTIME.runtime_id(), "databricks");
    }

    #[test]
    fn provider_kind_returns_databricks() {
        assert_eq!(DATABRICKS_RUNTIME.provider_kind(), "databricks");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(DATABRICKS_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://my-workspace.cloud.databricks.com"});
        let err = DATABRICKS_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "databricks-dbrx-instruct"});
        let err = DATABRICKS_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_http_base_url() {
        let config = json!({
            "model": "databricks-dbrx-instruct",
            "base_url": "http://my-workspace.cloud.databricks.com"
        });
        assert!(DATABRICKS_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_url_accepts_https() {
        assert!(DATABRICKS_RUNTIME
            .validate_endpoint_url("https://my-workspace.cloud.databricks.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(DATABRICKS_RUNTIME
            .validate_endpoint_url("http://my-workspace.cloud.databricks.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(DATABRICKS_RUNTIME
            .validate_endpoint_url("my-workspace.cloud.databricks.com")
            .is_err());
    }

    #[test]
    fn default_path_template_contains_model_placeholder() {
        let template = VerdictanRuntime::default_path_template(&DATABRICKS_RUNTIME).unwrap();
        assert_eq!(template, "/serving-endpoints/{model}/invocations");
    ...
    fn build_request_passes_through() {
        ...
        let result = DATABRICKS_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = DATABRICKS_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = DATABRICKS_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(DATABRICKS_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(DATABRICKS_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://myworkspace.databricks.com"});
        assert!(DATABRICKS_RUNTIME.validate_config(&config).is_err());
    }
