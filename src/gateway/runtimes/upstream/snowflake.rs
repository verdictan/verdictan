...
pub static SNOWFLAKE_RUNTIME: SnowflakeRuntime = SnowflakeRuntime;

pub struct SnowflakeRuntime;

impl super::super::VerdictanRuntime for SnowflakeRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for snowflake runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for SnowflakeRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "snowflake runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_snowflake() {
        assert_eq!(SNOWFLAKE_RUNTIME.runtime_id(), "snowflake");
    }

    #[test]
    fn provider_kind_returns_snowflake() {
        assert_eq!(SNOWFLAKE_RUNTIME.provider_kind(), "snowflake");
    }

    #[test]
    fn validate_config_accepts_valid() {
        let config =
            json!({"model": "llama3-8b", "base_url": "https://acct.snowflakecomputing.com"});
        assert!(SNOWFLAKE_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_http() {
        let config = json!({"model": "m", "base_url": "http://host"});
        assert!(SNOWFLAKE_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://acct.snowflake.com"});
        assert!(SNOWFLAKE_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_only_accepts_https() {
        assert!(SNOWFLAKE_RUNTIME
            .validate_endpoint_url("https://host.com")
            .is_ok());
        assert!(SNOWFLAKE_RUNTIME
            .validate_endpoint_url("http://host.com")
            .is_err());
    }

    #[test]
    fn default_path_template_is_cortex() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&SNOWFLAKE_RUNTIME),
            Some("/api/v2/cortex/inference:complete")
        );
    }
    ...
    fn supports_streaming_but_not_tools() {
        assert!(SNOWFLAKE_RUNTIME.supports_streaming());
        assert!(!SNOWFLAKE_RUNTIME.supports_tools());
    }

    #[test]
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://acct.snowflakecomputing.com"});
        assert!(SNOWFLAKE_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(SNOWFLAKE_RUNTIME
            .validate_endpoint_url("acct.snowflakecomputing.com")
            .is_err());
    }
    ...
    fn build_request_passes_through() {
        ...
        assert_eq!(
            SNOWFLAKE_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(SNOWFLAKE_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": []});
        assert_eq!(SNOWFLAKE_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
