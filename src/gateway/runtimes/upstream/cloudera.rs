...
pub static CLOUDERA_RUNTIME: ClouderaRuntime = ClouderaRuntime;

pub struct ClouderaRuntime;

impl super::super::VerdictanRuntime for ClouderaRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for cloudera runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for ClouderaRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "cloudera runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_cloudera() {
        assert_eq!(CLOUDERA_RUNTIME.runtime_id(), "cloudera");
    }

    #[test]
    fn provider_kind_returns_cloudera() {
        assert_eq!(CLOUDERA_RUNTIME.provider_kind(), "cloudera");
    }
    ...
    fn validate_config_accepts_valid_config() {
        let config = json!({
            "model": "llama-2-70b",
            "base_url": "https://ml.cloudera.example.com"
        });
        assert!(CLOUDERA_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://ml.cloudera.example.com"});
        let err = CLOUDERA_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama-2-70b"});
        let err = CLOUDERA_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_values() {
        let config = json!({"model": "", "base_url": ""});
        assert!(CLOUDERA_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_http_base_url() {
        let config = json!({
            "model": "llama-2-70b",
            "base_url": "http://ml.cloudera.example.com"
        });
        assert!(CLOUDERA_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(CLOUDERA_RUNTIME
            .validate_endpoint_url("https://ml.cloudera.example.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(CLOUDERA_RUNTIME
            .validate_endpoint_url("http://ml.cloudera.example.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(CLOUDERA_RUNTIME
            .validate_endpoint_url("ml.cloudera.example.com")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&CLOUDERA_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = CLOUDERA_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = CLOUDERA_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = CLOUDERA_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(CLOUDERA_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(CLOUDERA_RUNTIME.supports_tools());
    }
}
