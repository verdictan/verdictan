...
pub static COMETAPI_RUNTIME: CometApiRuntime = CometApiRuntime;

pub struct CometApiRuntime;

impl super::super::VerdictanRuntime for CometApiRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for cometapi runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for CometApiRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "cometapi runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_cometapi() {
        assert_eq!(COMETAPI_RUNTIME.runtime_id(), "cometapi");
    }

    #[test]
    fn provider_kind_returns_cometapi() {
        assert_eq!(COMETAPI_RUNTIME.provider_kind(), "cometapi");
    }
    ...
    fn validate_config_accepts_valid_config() {
        let config = json!({
            "model": "command-r",
            "base_url": "https://api.comet.example.com"
        });
        assert!(COMETAPI_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.comet.example.com"});
        let err = COMETAPI_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "  ", "base_url": "https://api.example.com"});
        assert!(COMETAPI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_http_base_url() {
        let config = json!({
            "model": "command-r",
            "base_url": "http://api.comet.example.com"
        });
        assert!(COMETAPI_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(COMETAPI_RUNTIME
            .validate_endpoint_url("https://api.comet.example.com")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(COMETAPI_RUNTIME
            .validate_endpoint_url("http://api.comet.example.com")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(COMETAPI_RUNTIME
            .validate_endpoint_url("api.comet.example.com")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&COMETAPI_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = COMETAPI_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        let config = json!({});
        let request = json!({"data": "x"});
        let result = COMETAPI_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = COMETAPI_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(COMETAPI_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(COMETAPI_RUNTIME.supports_tools());
    }
}
