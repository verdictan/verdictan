...
pub static JFROG_RUNTIME: JfrogRuntime = JfrogRuntime;

pub struct JfrogRuntime;

impl super::super::VerdictanRuntime for JfrogRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for jfrog runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for JfrogRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "jfrog runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_jfrog() {
        assert_eq!(JFROG_RUNTIME.runtime_id(), "jfrog");
    }

    #[test]
    fn provider_kind_returns_jfrog() {
        assert_eq!(JFROG_RUNTIME.provider_kind(), "jfrog");
    }

    #[test]
    fn validate_config_accepts_valid_config() {
        let config = json!({"model": "gpt-4", "base_url": "https://myorg.jfrog.io"});
        assert!(JFROG_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://myorg.jfrog.io"});
        let err = JFROG_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://myorg.jfrog.io"});
        assert!(JFROG_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_config_rejects_http_base_url() {
        let config = json!({"model": "gpt-4", "base_url": "http://myorg.jfrog.io"});
        assert!(JFROG_RUNTIME.validate_config(&config).is_err());
    }

    #[test]
    fn validate_endpoint_url_accepts_https() {
        assert!(JFROG_RUNTIME
            .validate_endpoint_url("https://myorg.jfrog.io")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(JFROG_RUNTIME
            .validate_endpoint_url("http://myorg.jfrog.io")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(JFROG_RUNTIME
            .validate_endpoint_url("myorg.jfrog.io")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&JFROG_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        assert_eq!(
            JFROG_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passes_through() {
        ...
        assert_eq!(JFROG_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passes_through() {
        let resp = json!({"choices": [{"message": {"content": "hello"}}]});
        assert_eq!(JFROG_RUNTIME.translate_response(&resp).unwrap(), resp);
    }
    ...
    fn normalize_upstream_response_passes_through() {
        ...
        assert_eq!(
            JFROG_RUNTIME.normalize_upstream_response(&resp).unwrap(),
            resp
        );
    }
    ...
    fn supports_streaming_is_true() {
        assert!(JFROG_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(JFROG_RUNTIME.supports_tools());
    }
}
