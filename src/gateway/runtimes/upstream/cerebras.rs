...
pub static CEREBRAS_RUNTIME: CerebrasRuntime = CerebrasRuntime;

pub struct CerebrasRuntime;

impl super::super::VerdictanRuntime for CerebrasRuntime {
            ...
            match config.get(key).and_then(Value::as_str).map(str::trim) {
                ...
                _ => {
                    return Err(CliError::user(format!(
                        "{key} is required for cerebras runtime"
                    )))
                }
            }
        }
...
impl VerdictanUpstreamRuntime for CerebrasRuntime {
    ...
    fn validate_endpoint_url(&self, base_url: &str) -> Result<(), CliError> {
        ...
        Err(CliError::user(
            "cerebras runtime requires an https:// base_url",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn runtime_id_returns_cerebras() {
        assert_eq!(CEREBRAS_RUNTIME.runtime_id(), "cerebras");
    }

    #[test]
    fn provider_kind_returns_cerebras() {
        assert_eq!(CEREBRAS_RUNTIME.provider_kind(), "cerebras");
    }
    ...
    fn validate_config_accepts_valid_config() {
        ...
        assert!(CEREBRAS_RUNTIME.validate_config(&config).is_ok());
    }

    #[test]
    fn validate_config_rejects_missing_model() {
        let config = json!({"base_url": "https://api.cerebras.ai"});
        let err = CEREBRAS_RUNTIME.validate_config(&config).unwrap_err();
        assert!(err.to_string().contains("model"));
    }

    #[test]
    fn validate_config_rejects_missing_base_url() {
        let config = json!({"model": "llama3-70b"});
        let err = CEREBRAS_RUNTIME.validate_config(&config).unwrap_err();
    ...
    fn validate_config_rejects_empty_model() {
        let config = json!({"model": "", "base_url": "https://api.cerebras.ai"});
        assert!(CEREBRAS_RUNTIME.validate_config(&config).is_err());
    }
    ...
    fn validate_endpoint_url_accepts_https() {
        assert!(CEREBRAS_RUNTIME
            .validate_endpoint_url("https://api.cerebras.ai")
            .is_ok());
    }
    ...
    fn validate_endpoint_url_rejects_http() {
        assert!(CEREBRAS_RUNTIME
            .validate_endpoint_url("http://api.cerebras.ai")
            .is_err());
    }
    ...
    fn validate_endpoint_url_rejects_bare_host() {
        assert!(CEREBRAS_RUNTIME
            .validate_endpoint_url("api.cerebras.ai")
            .is_err());
    }
    ...
    fn default_path_template_is_chat_completions() {
        assert_eq!(
            VerdictanRuntime::default_path_template(&CEREBRAS_RUNTIME),
            Some("/v1/chat/completions")
        );
    }
    ...
    fn build_request_passes_through() {
        ...
        let result = CEREBRAS_RUNTIME.build_request(&config, &input).unwrap();
    ...
    fn execute_passes_through() {
        ...
        let result = CEREBRAS_RUNTIME.execute(&config, &request).unwrap();
    ...
    fn translate_response_passes_through() {
        ...
        let result = CEREBRAS_RUNTIME.translate_response(&response).unwrap();
    ...
    fn supports_streaming_is_true() {
        assert!(CEREBRAS_RUNTIME.supports_streaming());
    }

    #[test]
    fn supports_tools_is_true() {
        assert!(CEREBRAS_RUNTIME.supports_tools());
    }
}
