...
pub static BEDROCK_AGENTS_RUNTIME: BedrockAgentsRuntime = BedrockAgentsRuntime;

pub struct BedrockAgentsRuntime;

impl super::super::VerdictanRuntime for BedrockAgentsRuntime {
...
impl VerdictanAgentRuntime for BedrockAgentsRuntime {
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(BEDROCK_AGENTS_RUNTIME.runtime_id(), "bedrock-agents");
    }

    #[test]
    fn validate_config_always_ok() {
        assert!(BEDROCK_AGENTS_RUNTIME.validate_config(&json!({})).is_ok());
        assert!(BEDROCK_AGENTS_RUNTIME
            .validate_config(&json!({"any": "thing"}))
            .is_ok());
    }

    #[test]
    fn requires_model_false() {
        assert!(!BEDROCK_AGENTS_RUNTIME.requires_model());
    }

    #[test]
    fn auth_optional_true() {
        assert!(BEDROCK_AGENTS_RUNTIME.auth_optional());
    }
    ...
    fn execute_passthrough() {
        let req = json!({"data": 1});
        assert_eq!(
            BEDROCK_AGENTS_RUNTIME.execute(&json!({}), &req).unwrap(),
            req
        );
    }
    ...
    fn translate_response_passthrough() {
        let resp = json!({"output": "hi"});
        assert_eq!(
            BEDROCK_AGENTS_RUNTIME.translate_response(&resp).unwrap(),
            resp
        );
    }
    ...
    fn initialize_agent_returns_empty_object() {
        let state = BEDROCK_AGENTS_RUNTIME.initialize_agent(&json!({})).unwrap();
        assert_eq!(state, json!({}));
    }

    #[test]
    fn execute_agent_call_passthrough() {
        let req = json!({"tool": "search"});
        let result = BEDROCK_AGENTS_RUNTIME
    ...
    fn stream_agent_events_empty() {
        let events = BEDROCK_AGENTS_RUNTIME
    ...
    fn finalize_agent_state_passthrough() {
        let state = json!({"session": "abc"});
        assert_eq!(
            BEDROCK_AGENTS_RUNTIME.finalize_agent_state(&state).unwrap(),
            state
        );
    }
    ...
    fn does_not_support_streaming() {
        assert!(!BEDROCK_AGENTS_RUNTIME.supports_streaming());
    }

    #[test]
    fn does_not_support_tools() {
        assert!(!BEDROCK_AGENTS_RUNTIME.supports_tools());
    }
}
