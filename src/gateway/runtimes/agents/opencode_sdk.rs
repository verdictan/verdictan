...
pub static OPENCODE_SDK_RUNTIME: OpenCodeSdkRuntime = OpenCodeSdkRuntime;

pub struct OpenCodeSdkRuntime;

impl super::super::VerdictanRuntime for OpenCodeSdkRuntime {
...
impl VerdictanAgentRuntime for OpenCodeSdkRuntime {
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(OPENCODE_SDK_RUNTIME.runtime_id(), "opencode-sdk");
    }

    #[test]
    fn validate_config_always_ok() {
        assert!(OPENCODE_SDK_RUNTIME.validate_config(&json!({})).is_ok());
    }

    #[test]
    fn requires_model_false() {
        assert!(!OPENCODE_SDK_RUNTIME.requires_model());
    }

    #[test]
    fn auth_optional_true() {
        assert!(OPENCODE_SDK_RUNTIME.auth_optional());
    }

    #[test]
    fn initialize_agent_returns_empty_object() {
        assert_eq!(
            OPENCODE_SDK_RUNTIME.initialize_agent(&json!({})).unwrap(),
            json!({})
        );
    }
    ...
    fn execute_agent_call_passthrough() {
        let req = json!({"task": "refactor"});
        assert_eq!(
            OPENCODE_SDK_RUNTIME
    ...
    fn finalize_agent_state_passthrough() {
        let state = json!({"session": "s-1"});
        assert_eq!(
            OPENCODE_SDK_RUNTIME.finalize_agent_state(&state).unwrap(),
            state
        );
    }
    ...
    fn execute_passthrough() {
        let req = json!({"data": 1});
        assert_eq!(OPENCODE_SDK_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }
    ...
    fn translate_response_passthrough() {
        ...
        assert_eq!(
            OPENCODE_SDK_RUNTIME.translate_response(&resp).unwrap(),
            resp
        );
    }
    ...
    fn does_not_support_streaming() {
        assert!(!OPENCODE_SDK_RUNTIME.supports_streaming());
    }

    #[test]
    fn does_not_support_tools() {
        assert!(!OPENCODE_SDK_RUNTIME.supports_tools());
    }
}
