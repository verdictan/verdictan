...
pub static CODEX_SDK_RUNTIME: CodexSdkRuntime = CodexSdkRuntime;

pub struct CodexSdkRuntime;

impl super::super::VerdictanRuntime for CodexSdkRuntime {
...
impl VerdictanAgentRuntime for CodexSdkRuntime {
...
mod tests {
    ...
    #[test]
    fn runtime_id() {
        assert_eq!(CODEX_SDK_RUNTIME.runtime_id(), "codex-sdk");
    }

    #[test]
    fn validate_config_always_ok() {
        assert!(CODEX_SDK_RUNTIME.validate_config(&json!({})).is_ok());
    }

    #[test]
    fn requires_model_false() {
        assert!(!CODEX_SDK_RUNTIME.requires_model());
    }

    #[test]
    fn auth_optional_true() {
        assert!(CODEX_SDK_RUNTIME.auth_optional());
    }

    #[test]
    fn initialize_agent_returns_empty_object() {
        assert_eq!(
            CODEX_SDK_RUNTIME.initialize_agent(&json!({})).unwrap(),
            json!({})
        );
    }
    ...
    fn finalize_agent_state_passthrough() {
        let state = json!({"result": "ok"});
        assert_eq!(
            CODEX_SDK_RUNTIME.finalize_agent_state(&state).unwrap(),
            state
        );
    }
    ...
    fn build_request_passthrough() {
        ...
        assert_eq!(
            CODEX_SDK_RUNTIME.build_request(&json!({}), &input).unwrap(),
            input
        );
    }
    ...
    fn execute_passthrough() {
        let req = json!({"data": 1});
        assert_eq!(CODEX_SDK_RUNTIME.execute(&json!({}), &req).unwrap(), req);
    }

    #[test]
    fn translate_response_passthrough() {
        let resp = json!({"output": "hello"});
        assert_eq!(CODEX_SDK_RUNTIME.translate_response(&resp).unwrap(), resp);
    }

    #[test]
    fn does_not_support_streaming() {
        assert!(!CODEX_SDK_RUNTIME.supports_streaming());
    }

    #[test]
    fn does_not_support_tools() {
        assert!(!CODEX_SDK_RUNTIME.supports_tools());
    }
}
