...
//! MCP resource for recent context entries.
...
const RESOURCE_URI: &str = "context://recent";

pub(crate) fn descriptor() -> Value {
    serde_json::json!({
        "uri": RESOURCE_URI,
        "name": "Recent Context",
        "description": "Recent context entries from /v1/context/recent. Optional query params: session_id, team_id, repo, branch, limit, cursor.",
...
pub(crate) async fn read_resource_for_session(
    ...
    if !matches_uri(uri) {
        return Err(CliError::user(format!(
            "Unknown recent context resource URI: {uri}"
        )));
    }
    ...
    tracing::debug!(uri = %uri, path = %path, "reading recent context MCP resource");
...
fn build_recent_path(
    ...
    if let Some(value) = limit.filter(|value| !value.trim().is_empty()) {
        value.parse::<u64>().map_err(|_| {
            CliError::user("context://recent 'limit' query parameter must be a positive integer")
        })?;
