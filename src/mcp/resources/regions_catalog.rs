...
//! MCP resource for the public regions catalog.
...
const RESOURCE_URI: &str = "regions://catalog";
const LEGACY_RESOURCE_URI: &str = "regions-catalog://public";

pub(crate) fn descriptor() -> Value {
    serde_json::json!({
        "uri": RESOURCE_URI,
        "name": "Regions Catalog",
        "description": "Public region catalog from /v1/regions.",
...
pub(crate) async fn read_resource(client: &AsyncApiClient, uri: &str) -> Result<Value, CliError> {
    if !matches_uri(uri) {
        return Err(CliError::user(format!(
            "Unknown regions catalog resource URI: {uri}"
        )));
    }

    tracing::debug!(uri = %uri, "reading regions catalog MCP resource");

    let response = client.get_json_value("/v1/regions").await?;
...
mod tests {
    ...
    async fn regions_handler(State(state): State<RegionsApiState>) -> impl IntoResponse {
    ...
    async fn spawn_regions_api(response: Value) -> (AsyncApiClient, tokio::task::JoinHandle<()>) {
        ...
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("bind regions api");
        let addr = listener.local_addr().expect("regions api addr");
        let handle = tokio::spawn(async move {
            axum::serve(listener, app).await.expect("serve regions api");
        });
        let client =
            AsyncApiClient::new(format!("http://{addr}"), "test-token").expect("regions client");
        (client, handle)
    }
    ...
    #[tokio::test]
    async fn read_resource_serializes_regions_array() {
        ...
        assert_eq!(payload["regions"][0]["region_key"], "eu-west");
    ...
    async fn read_resource_falls_back_to_empty_array() {
        ...
        assert_eq!(payload["regions"], serde_json::json!([]));
